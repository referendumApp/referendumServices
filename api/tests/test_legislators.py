...
def test_legislator_workflow():
    ...
    response = client.put("/legislators", json=original_legislator_payload, headers=system_headers)
    ...
    get_response = client.get(f"/legislators/{legislator_id}", headers=system_headers)
    assert get_response.status_code == 200
    assert created_legislator["state"] == "WA"

    # Step 3: Update the legislator (change the chamber)
    created_legislator["chamber"] = "senate"
    update_response = client.post("/legislators", json=created_legislator, headers=system_headers)
    assert update_response.status_code == 200

    # Step 4: Get the legislator again after the update to verify the changes
    get_response_after_update = client.get(f"/legislators/{legislator_id}", headers=system_headers)
    ...
    delete_response = client.delete(f"/legislators/{legislator_id}", headers=system_headers)
    ...
    get_response_after_delete = client.get(f"/legislators/{legislator_id}", headers=system_headers)
    assert get_response_after_delete.status_code == 404

    # Step 7: Try to get a non-existent bill
    non_existent_legislator_id = 99999
    get_non_existent = client.get(f"/legislators/{non_existent_legislator_id}", headers=system_headers)
