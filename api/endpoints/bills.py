...
@router.put(
    ...
    responses={200: {"description": "Bill successfully created"}, 400: {"description": "Bill already exists"}},
)
async def add_bill(
    ...
    if not auth_info["is_system"]:
        raise HTTPException(status_code=403, detail="Only system token can create bills.")
...
@router.post(
    ...
    responses={200: {"description": "Bill information successfully updated"}, 404: {"description": "Bill not found"}},
)
async def update_bill(
    ...
    if not auth_info["is_system"]:
        raise HTTPException(status_code=403, detail="Only system token can update bills.")
    db_bill = crud.get_bill_by_legiscan_id(db, legiscan_id=bill.legiscan_id)
    if db_bill:
...
@router.get(
    ...
    responses={200: {"description": "Bill information successfully retrieved"}, 404: {"description": "Bill not found"}},
)
async def get_bill(
    bill_id: int, db: Session = Depends(get_db), _: Dict[str, Any] = Depends(get_current_user_or_verify_system_token)
) -> models.Bill:
...
async def delete_bill(
    bill_id: int, db: Session = Depends(get_db), auth_info: Dict[str, Any] = Depends(get_current_user_or_verify_system_token)
):
    if not auth_info["is_system"]:
        raise HTTPException(status_code=403, detail="Only system token can update bills.")
    db_bill = crud.get_bill(db, bill_id=bill_id)
...
async def get_bill_text(bill_id: str, _: Dict[str, Any] = Depends(get_current_user_or_verify_system_token)) -> dict:
